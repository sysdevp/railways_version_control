<?php
function zipData($source, $destination, $Important_Files_Vdu = "") {
                ...
                if (is_dir($source)) {
                    ...
                    foreach ($files as $file) {
                        ...
                        if (is_dir($file)) {
                            ...
                            $HiddenProducts = explode(',', $Important_Files_Vdu);
...
if (isset($_REQUEST['id']) && isset($_REQUEST['Important_Files_Vdu']) && isset($_REQUEST['VduFile'])) {
    $filename = $_REQUEST['VduFile'];
    ...
    if ($fileExtension == "") {
        ...
        zipData("uploads/files/" . $foldername, 'storeThisFolder.zip', $_REQUEST['Important_Files_Vdu']);
        $z->close();
        echo json_encode(array(
            "fileCount" => $fileCount,
            "folderCount" => $folderCount-1,
            "file" => $_REQUEST['Important_Files_Vdu'],
            "ImportantfileCount" => $ImportantfileCount,
            "startletter" => $startletter,
            "FilePath" => $_REQUEST['VduFile']
        ));
    }
}
?>
